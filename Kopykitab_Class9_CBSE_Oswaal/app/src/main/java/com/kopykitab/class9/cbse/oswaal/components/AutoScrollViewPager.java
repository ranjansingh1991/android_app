...
/**
 * AutoScrollViewPager: A layout that displays a ViewPager with its children that are outside
...
public class AutoScrollViewPager extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    private void init() {
        ...
        handler = new MyHandler(AutoScrollViewPager.this);
    ...
    public void onPageSelected(int position) {
        if (currentPosition < position) {
            ...
            PagerAdapter adapter = getViewPager().getAdapter();
            if (position == 0) {
                getViewPager().setCurrentItem(adapter.getCount() - 2, false);
            }
        }
    ...
    public void scrollOnce() {
        ...
        if (nextItem < 1) {
            if (isCycle) {
                getViewPager().setCurrentItem(totalCount - 2, isBorderAnimation);
            }
        } else if (nextItem == totalCount - 1) {
            if (isCycle) {
                getViewPager().setCurrentItem(1, false);
            }
        } else {
            getViewPager().setCurrentItem(nextItem, true);
        }
    }
        ...
        if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT || slideBorderMode == SLIDE_BORDER_MODE_CYCLE) {
            ...
            if ((currentItem == 0 && downX <= touchX) || (currentItem == pageCount - 1 && downX >= touchX)) {
                if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT) {
                    ...
                    if (pageCount > 1) {
                        getViewPager().setCurrentItem(pageCount - currentItem - 1, isBorderAnimation);
                    }
