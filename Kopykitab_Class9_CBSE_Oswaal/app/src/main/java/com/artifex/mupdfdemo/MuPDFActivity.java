...
import com.kopykitab.class9.cbse.oswaal.components.adapters.FindMoreBooksAdapter;
import com.kopykitab.class9.cbse.oswaal.components.adapters.MuPdfItemClickListener;
...
public class MuPDFActivity extends AppCompatActivity implements FilePicker.FilePickerSupport, PopoverViewDelegate, MuPdfItemClickListener {
    ...
    private ImageButton mShareThisBookButton;
    ...
    private static boolean isNotesDeleted = false;
    private PopupWindow moreAnnotationsDropdown = null;
    private CircularImageView btnSlideUp;
    private Button btnSlidCls;
    private LinearLayout findMoreBookLayout;
    private ArrayList<Book> BookItemList = new ArrayList<>();
    private RecyclerView mFindMoreView;
    private FindMoreBooksAdapter mFindMoreAdapter;
    private View transitionsContainer;
    private boolean isAlsoBoughtWigetVisible = false, isAlsoBoughtButtonVisible = true, isAlsoBoughtButtonVisibleFirstTime = true;
    private boolean btnOri1 = false;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (core == null) {
            ...
            if (savedInstanceState != null && savedInstanceState.containsKey("FileName")) {
                ...
                isAlsoBoughtWigetVisible = savedInstanceState.getBoolean("isAlsoBoughtWigetVisible");
                btnOri1 = savedInstanceState.getBoolean("btnOri1");

                BookItemList = savedInstanceState.getParcelableArrayList("abc");
                isAlsoBoughtButtonVisible = savedInstanceState.getBoolean("isAlsoBoughtButtonVisible");
                isAlsoBoughtButtonVisibleFirstTime = savedInstanceState.getBoolean("isAlsoBoughtButtonVisibleFirstTime");
            }
        }
        if (core == null) {
    ...
    public void createUI(Bundle savedInstanceState) {
        ...
        mShareThisBookButton.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (Integer.parseInt(productId) > 0) {
                    Utils.triggerGAEvent(MuPDFActivity.this, "Pdf_Share", Constants.LOGIN_SOURCE + "_" + AppSettings.getInstance(MuPDFActivity.this).get("CUSTOMER_ID"), productId);

                    List<Intent> targetShareIntents = new ArrayList<Intent>();
                    Intent shareIntent = new Intent();
                    shareIntent.setAction(Intent.ACTION_SEND);
                    shareIntent.setType("text/plain");
                    String message = "I found " + book.get("name") + " very good for my studies for " + Constants.FORMATTED_LOGIN_SOURCE + "\n\n";
                    message += "You can also read it absolutely FREE!\n\nDownload the book now using this app:\n";
                    message += "https://play.google.com/store/apps/details?id=" + getApplicationContext().getPackageName();

                    List<ResolveInfo> resInfos = getPackageManager().queryIntentActivities(shareIntent, 0);
                    if (!resInfos.isEmpty()) {
                        for (ResolveInfo resInfo : resInfos) {
                            String packageName = resInfo.activityInfo.packageName;
                            if (packageName.contains("whatsapp") || packageName.contains("facebook") || packageName.contains("sms") || packageName.contains("android.gm") || packageName.contains("talk") || packageName.contains("inbox")) {
                                Intent intent = new Intent();
                                intent.setComponent(new ComponentName(packageName, resInfo.activityInfo.name));
                                intent.setAction(Intent.ACTION_SEND);
                                intent.setType("text/plain");
                                intent.putExtra(Intent.EXTRA_TEXT, message);
                                if (packageName.contains("android.gm") || packageName.contains("inbox")) {
                                    intent.putExtra(Intent.EXTRA_SUBJECT, "Best Book for studies! It's FREE");
                        ...
                        if (!targetShareIntents.isEmpty()) {
                            Intent chooserIntent = Intent.createChooser(targetShareIntents.remove(0), "Share this book its FREE :");
    ...
    public void itemClicked(Book book) {
        String url = book.getHref();
        if (!url.equals("")) {
            Utils.showWebViewActivity(MuPDFActivity.this, url);
            Utils.triggerGAEventOnline(MuPDFActivity.this, "Related_Products_Item", book.getProductId(), customerId);
        }
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (mFileName != null && mDocView != null) {
            ...
            outState.putBoolean("isAlsoBoughtWigetVisible", isAlsoBoughtWigetVisible);
            outState.putBoolean("btnOri1", btnOri1);
            outState.putBoolean("isAlsoBoughtButtonVisible", isAlsoBoughtButtonVisible);
            outState.putBoolean("isAlsoBoughtButtonVisibleFirstTime", isAlsoBoughtButtonVisibleFirstTime);
            outState.putParcelableArrayList("abc", BookItemList);
    ...
    private void makeButtonsView() {
        ...
        mShareThisBookButton = (ImageButton) mButtonsView.findViewById(R.id.shareThisBookButton);
        ...
        findMoreBookLayout = (LinearLayout) mButtonsView.findViewById(R.id.find_more_book_layout);
        btnSlideUp = (CircularImageView) mButtonsView.findViewById(R.id.slide_page_open);
        btnSlidCls = (Button) mButtonsView.findViewById(R.id.find_more_close_button);
        mFindMoreView = (RecyclerView) mButtonsView.findViewById(R.id.find_more_book_view);
        transitionsContainer = (View) mButtonsView.findViewById(R.id.hidden_layout);
        mFindMoreAdapter = new FindMoreBooksAdapter(this, BookItemList, this);


        // horizontal RecyclerView
        RecyclerView.LayoutManager mLayoutManager =
                new LinearLayoutManager(getApplicationContext(), LinearLayoutManager.HORIZONTAL, false);
        mFindMoreView.setLayoutManager(mLayoutManager);
        mFindMoreView.setItemAnimator(new DefaultItemAnimator());
        mFindMoreView.setAdapter(mFindMoreAdapter);

        if (!productId.equals("-1") && Utils.isNetworkConnected(this)) {
            if (isAlsoBoughtWigetVisible) {
                transitionsContainer.bringToFront();
            ...
            if (isAlsoBoughtButtonVisible && isAlsoBoughtButtonVisibleFirstTime) {
            ...
            if (BookItemList != null && BookItemList.size() > 0) {
                mFindMoreAdapter = new FindMoreBooksAdapter(this, BookItemList, this);
                mFindMoreView.setAdapter(mFindMoreAdapter);
                mFindMoreAdapter.notifyDataSetChanged();
            } else {
                new prepareBookDataFromApi().execute();
            }
        }
        ...
        btnSlideUp.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Utils.triggerGAEventOnline(MuPDFActivity.this, "Related_Products", productId, customerId);
                transitionsContainer.setAnimation(AnimationUtils.loadAnimation(MuPDFActivity.this, R.anim.bottom_up));
                Animation anim = new TranslateAnimation(0, 0, transitionsContainer.getHeight(), 0);
                anim.setDuration(500);
                anim.setAnimationListener(new Animation.AnimationListener() {
                    public void onAnimationStart(Animation animation) {
                        transitionsContainer.bringToFront();
                        transitionsContainer.setVisibility(View.VISIBLE);
                        isAlsoBoughtWigetVisible = true;
                        isAlsoBoughtButtonVisible = false;
                        isAlsoBoughtButtonVisibleFirstTime = false;
                        btnOri1 = false;
                    }
                ...
                transitionsContainer.startAnimation(anim);
                btnSlideUp.setVisibility(View.GONE);
            }
        });


        btnSlidCls.setOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View v) {
                Animation anim = new TranslateAnimation(0, 0, 0, 0);
                transitionsContainer.setAnimation(AnimationUtils.loadAnimation(MuPDFActivity.this, R.anim.bottom_down));
                anim.setDuration(500);
                anim.setAnimationListener(new Animation.AnimationListener() {
                    public void onAnimationStart(Animation animation) {
                        transitionsContainer.setVisibility(View.GONE);
                        isAlsoBoughtWigetVisible = false;
                        isAlsoBoughtButtonVisible = true;
                        btnOri1 = false;
                    }
    ...
    public void finish() {
        ...
        if (Utils.isNetworkConnected(MuPDFActivity.this)) {
            ...
            String recommendationShowTimeString = AppSettings.getInstance(MuPDFActivity.this).get("recommendation_shown_time");
            long recommendationShowTime = 0;
            if (recommendationShowTimeString != null && !recommendationShowTimeString.equals("") && !recommendationShowTimeString.isEmpty()) {
                recommendationShowTime = Long.parseLong(recommendationShowTimeString);
            }
            String recommendationShowTimeIntervalString = AppSettings.getInstance(MuPDFActivity.this).get("customer_recommendations_interval");
            long recommendationShowTimeInterval = 0;
            if (recommendationShowTimeIntervalString != null && !recommendationShowTimeIntervalString.equals("") && !recommendationShowTimeIntervalString.isEmpty()) {
                recommendationShowTimeInterval = Long.parseLong(recommendationShowTimeIntervalString);
            }
            long recommendationShowNextTime = recommendationShowTime + (recommendationShowTimeInterval * 1000);
            if (System.currentTimeMillis() >= recommendationShowNextTime) {
                AppSettings.getInstance(MuPDFActivity.this).set("recommendation_shown_time", "" + System.currentTimeMillis());

                Intent webViewIntent = new Intent(MuPDFActivity.this, WebViewActivity.class);
                webViewIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                webViewIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                webViewIntent.putExtra("web_url", Constants.PRODUCT_RECOMMENDATIONS_URL + "&product_id=" + productId);
    ...
    public class NotesAndBookmarkPagerAdapter extends FragmentStatePagerAdapter {
        ...
        public Fragment getItem(int position) {
            ...
            if (fragments[position] == null) {
                fragments[position] = new DetailedNotesOrBookmarksFragment(mContext, position, mChapterInfo, book);
            }
    ...
    public final static class UtilsPDF {
        ...
        public static void setIsNotesDeleted(boolean isDeleted) {
            isNotesDeleted = isDeleted;
        }
    }

    private class prepareBookDataFromApi extends AsyncTask<Void, Void, Void> {

        @Override
        protected Void doInBackground(Void... params) {
            try {
                String response = Utils.sendPost(Constants.FIND_MORE_URL + "&product_id=" + URLEncoder.encode(productId, "UTF-8"), "customer_id=" + URLEncoder.encode(customerId, "UTF-8") + "&login_source=" + URLEncoder.encode(Constants.LOGIN_SOURCE, "UTF-8") + "&render_json=" + URLEncoder.encode("true", "UTF-8"));
                if (response != null) {
                    JSONArray findMoreBooksJsonArray = new JSONArray(response);
                    if (findMoreBooksJsonArray.length() > 0) {
                        Book book;
                        for (int i = 0; i < findMoreBooksJsonArray.length(); i++) {
                            JSONObject books = findMoreBooksJsonArray.getJSONObject(i);
                            book = new Book(books.getString("product_id").trim(), books.getString("image"), books.getString("name").trim(), books.getString("price_1"), books.getString("price_2"), books.getString("url"));
