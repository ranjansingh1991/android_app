...
public abstract class PageView extends ViewGroup {
    ...
    public PageView(Context c, Point parentSize, Bitmap sharedHqBm) {
        ...
        BACKGROUND_COLOR = ((ColorDrawable) ((Activity) c).getWindow().getDecorView().getBackground()).getColor();
    ...
    public void setPage(int page, PointF size, final String productId) {
        // Cancel pending render task
        if (mDrawEntire != null) {
            mDrawEntire.cancelAndWait();
            mDrawEntire = null;
        ...
        if (mSearchView == null) {
            mSearchView = new View(mContext) {
                Bitmap icon = null, iconScaled = null;
                float scaleValue = 0;
                ...
                protected void onDraw(final Canvas canvas) {
                    ...
                    scaleValue = scale;
                    String customerId = AppSettings.getInstance(getContext()).get("CUSTOMER_ID");
                    ...
                    //get all notes and their coordinates by key
                    HashMap<String, HashMap<String, String>> allNotesAndCoordinates = UtilsPDF.getAllNotesAndCoordinatesByKey(getContext(), customerId, productId, pageNumber);
                    if (allNotesAndCoordinates != null && allNotesAndCoordinates.size() > 0) {
                        Set<String> coordinates = allNotesAndCoordinates.keySet();
                        for (String coord : coordinates) {
                            HashMap<String, String> notesData = allNotesAndCoordinates.get(coord);
                            String notes = notesData.get("notes");
                            if (notes != null && !notes.isEmpty()) {
                                String coordString[] = coord.split(",");
                                coordString[0] = coordString[0].trim();
                                coordString[1] = coordString[1].trim();
                                coordString[2] = coordString[2].trim();

                                int key = (int) Float.parseFloat(coordString[0]);
                                float xFloat = (Float.parseFloat(coordString[1]) - 14) * scale;
                                float yFloat = (Float.parseFloat(coordString[2]) - 55) * scale;

                                icon = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.annots_notes_indicator);
                                int sizeX = (int) getResources().getDimension(R.dimen.notes_icon_weight);
                                int sizeY = (int) getResources().getDimension(R.dimen.notes_icon_height);
                                iconScaled = Bitmap.createScaledBitmap(icon, sizeX, sizeY, false);

                                if (key == Integer.valueOf(Annotation.Type.HIGHLIGHT.ordinal())) {
                                    paint.setColor(0x75FFFF00);
                                } else if (key == Integer.valueOf(Annotation.Type.UNDERLINE.ordinal())) {
                                    paint.setColor(0xFF0000FF);
                                } else if (key == Integer.valueOf(Annotation.Type.STRIKEOUT.ordinal())) {
                                    paint.setColor(0xFFFF0000);
                                }

                                paint.setStyle(Paint.Style.FILL);
                                canvas.drawBitmap(iconScaled, xFloat, yFloat, paint);
                            }
                        }
                    }
                }
                ...
                public boolean onTouchEvent(MotionEvent event) {
                    ...
                    final String customerId = AppSettings.getInstance(getContext()).get("CUSTOMER_ID");
                    final Integer pageNumber = getPage();
                    float x = event.getX();
                    float y = event.getY();
                    switch (event.getAction()) {
                        case MotionEvent.ACTION_DOWN:
                            ...
                            if (allNotesAndCoordinates != null && allNotesAndCoordinates.size() > 0) {
                                Set<String> coordinates = allNotesAndCoordinates.keySet();
                                for (final String coord : coordinates) {

                                    String coordString[] = coord.split(",");
                                    float xFloat = (Float.parseFloat(coordString[1]) - 14) * scaleValue;
                                    float yFloat = (Float.parseFloat(coordString[2]) - 55) * scaleValue;

                                    //Check if the x and y position of the touch is inside the bitmap
                                    if (iconScaled != null) {
                                        if ((x > xFloat && x < xFloat + iconScaled.getWidth() && y > yFloat && y < yFloat + iconScaled.getHeight())) {
                                            //Bitmap touched
                                            final AlertDialog alertDialog = UtilsPDF.createAlertBox(getContext());
                                            alertDialog.show();
                                            ...
                                            saveNotesButton.setOnClickListener(new OnClickListener() {
                                                ...
                                                public void onClick(View v) {
                                                    // TODO Auto-generated method stub
                                                    String notesMsg = editText.getText().toString();
                                                    CharSequence text = "";
                                                    if (!notesMsg.isEmpty()) {
                                                        ...
                                                        UtilsPDF.saveNotes(getContext(), notesData, customerId, productId, pageNumber, coord);
