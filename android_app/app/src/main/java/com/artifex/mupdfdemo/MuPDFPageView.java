...
public class MuPDFPageView extends PageView implements MuPDFView {
    ...
    public boolean copySelection() {
        ...
        if (currentApiVersion >= android.os.Build.VERSION_CODES.HONEYCOMB) {
            ...
            cm.setPrimaryClip(ClipData.newPlainText("KopyKitab eReader", text));
        } else {
    ...
    public StringBuilder copySelectionText() {
        final StringBuilder text = new StringBuilder();
    ...
    public List<Float> copySelectionCoordinates(Annotation.Type type) {
        ...
        return mCore.setLastAddedAnnotation(quadPoints, type);
    }
    ...
    public void deleteSelectedAnnotation() {
        if (mSelectedAnnotationIndex != -1) {
            ...
            //delete Notes
            mCore.deleteNotesAnnotation(mPageNumber, mSelectedAnnotationIndex);

            mDeleteAnnotation = new AsyncTask<Integer, Void, Void>() {
                @Override
                protected Void doInBackground(Integer... params) {
                    mCore.deleteAnnotation(mPageNumber, params[0], mAnnotations[params[0]].type);
                    return null;
                }
    ...
    public void loadAnnotations() {
        ...
        //load external annotation from JSON file
        loadAnnotationsFromJSONFile();
    ...
    public void loadAnnotationsFromJSONFile() {
        try {
            Set<List<Float>> allAnnotations = mCore.getAnnotationsByPageNumber(mPageNumber);
            if (allAnnotations != null && allAnnotations.size() > 0) {
                for (List<Float> coordinates : allAnnotations) {
                    Integer annotationKey = Float.valueOf(coordinates.get(0)).intValue();
                    List<PointF> annotationPointF = new ArrayList<PointF>();
                    for (int i = 1; i < coordinates.size(); i += 2) {
                        annotationPointF.add(new PointF(coordinates.get(i), coordinates.get(i + 1)));
                    }
                    PointF[] annotationPointFArray = annotationPointF.toArray(new PointF[annotationPointF.size()]);
                    if (annotationKey.equals(Integer.valueOf(Annotation.Type.HIGHLIGHT.ordinal()))) {
                        mCore.addMarkupAnnotation(mPageNumber, annotationPointFArray, Annotation.Type.HIGHLIGHT);
                    } else if (annotationKey.equals(Integer.valueOf(Annotation.Type.UNDERLINE.ordinal()))) {
                        mCore.addMarkupAnnotation(mPageNumber, annotationPointFArray, Annotation.Type.UNDERLINE);
                    } else if (annotationKey.equals(Integer.valueOf(Annotation.Type.STRIKEOUT.ordinal()))) {
                        mCore.addMarkupAnnotation(mPageNumber, annotationPointFArray, Annotation.Type.STRIKEOUT);
                    }
                }
            }
        } catch (Exception e) {
    ...
    @Override
    public void setPage(final int page, PointF size, final String productId) {
        loadAnnotations();
        ...
        super.setPage(page, size, productId);
    }
